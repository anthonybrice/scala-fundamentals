...
class TypesOfLoopsSuite extends FunSuite {
  ...
  test("fold with builder") {
    ...
    val EmptyColorAliasBuilder = ColorAliasBuilder("<none>", Map().withDefaultValue(Seq()))

    //for a fold, we have to define the transition from the current aggregator and the next element, to the new aggregator
    //in this case our aggregator is the builder, all of the state goes here
    //our element is the line
    def updateBuilderWithLine(aggregator: ColorAliasBuilder, element: String): ColorAliasBuilder = {
      element match {
        case ColorPattern(colorName) => aggregator.setCurrentColor(colorName)
        case AliasPattern(aliasName) => aggregator.addAlias(aliasName)
      }
    }

    //here we do the fold
    //the first parameter list contains the initial value for our aggregator
    //the second parameter list contains the function that takes the old aggregator and current line and returns a new aggregator
    val builder = colorAliasLines.foldLeft(EmptyColorAliasBuilder)(updateBuilderWithLine)

    //we only grab the part of the aggregator we care about
  ...
  test("recursion") {
    ...
    //define the initial value for our aggregator
